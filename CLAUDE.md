...
- `unstudyCard` («В беклог» в редакторе) стирает `srs` — карточка выпадает из
  очереди и снова видна в Backlog своей линии. Старое состояние FSRS уезжает
  внутрь события `backlog` в логе: назад всё равно только через `introduce`,
  который сеет заново, но история оценок без расписания, которое она породила,
  неповторяема. Карточка **вне линий** после этого не видна нигде — Backlog
  показывает пул линии; такие перечислены внизу `/account`.
- `flagCard` — звёздочка ★ (`cards.flaggedAt`, метка времени, не флаг): пометить
  карточку прямо в сессии, чтобы вернуться позже. Пишется сразу, мимо формы, и
  события в лог не пишет — поле и есть всё состояние. Читают её два места: ★ на
  Learn и `search_cards({ flagged: true })` в MCP, где свежая пометка идёт
  первой.
