...
Секрет в URL — это вся авторизация. Кто узнал ссылку, тот читает колоду и
правит текст карточек (`edit_card`).
...
Читает: `search_cards`, `get_card`, `review_history`. Пишет одно — `edit_card`
правит текст карточки (стороны и `note`) и больше ничего: расписание, линии и
примеры не трогает. Остальная пишущая половина — план в `PLAN-inbox.md`.
