...
`server/mcp.ts`, MCP поверх колоды (читает колоду, правит текст карточек и
заводит новые).
...
правит текст карточек (`edit_card`) и заводит новые (`create_card`).
...
Читает: `search_cards`, `get_card`, `review_history`. Пишет двумя тулами:
`edit_card` правит текст карточки (стороны и `note`) и больше ничего —
расписание, линии и примеры не трогает; `create_card` заводит новую карточку в
начало линии, без расписания (она попадает в пул новых на Deck, в учёбу её
берут руками) и с отбивкой дублей по стороне A. Остальная пишущая половина —
план в `PLAN-inbox.md`.
