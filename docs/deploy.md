...
заводит новые и вешает на них примеры).
...
правит текст карточек (`edit_card`), заводит новые (`create_card`) и правит
примеры (`add_example`, `edit_example`).
...
Читает: `search_cards`, `get_card`, `review_history`, `search_examples`.

Пишет: `edit_card` правит текст карточки (стороны и `note`) и больше ничего —
расписание, линии и примеры не трогает; `create_card` заводит новую карточку в
начало линии, сразу в оборот (с расписанием новой карточки — спросят в
ближайшей сессии) и с отбивкой дублей по стороне A; `add_example` вешает
предложение на карточку (новое или уже лежащее в колоде — по `exampleId`),
`edit_example` правит само предложение и пропуски одной привязки. Пропуски
задаются позициями слов (`blankWords`, нумерация приходит в поле `words`) —
так же, как в приложении кликают по словам.

Не умеет ничего удалять — ни карточку, ни пример, ни привязку. Это осознанно:
удаление примера каскадом уносит все его привязки, и отката из чата нет.
Остальная пишущая половина — план в `PLAN-inbox.md`.
